...
function CH = homogenize_2d_andreassen(lx, ly, lambda, mu, phi, x, ignore_void)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% This function was taken from the paper:
%% Andreassen, E., & Andreasen, C. S. (2014).
...
% ignore_void = Ignore void material elements (Warning: requires connected solid connected)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% INITIALIZE
...
lambda = lambda(1)*(x==0) + lambda(2)*(x==1);
mu     = mu(1)*(x==0) + mu(2)*(x==1);
...
if ignore_void
    ...
    chi = zeros(ndof,3);
    chi(activedofs(3:end),:) = K(activedofs(3:end),activedofs(3:end))\F(activedofs(3:end),:);
    clear activedofs
else
...
disp('--- Homogenized elasticity tensor (Voigt) ---'); disp(CH)
endfunction
